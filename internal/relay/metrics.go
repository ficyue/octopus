...
func (m *RelayMetrics) Save(ctx context.Context, success bool, err error, attempts []model.ChannelAttempt) {
	...
	// TPS: tokens per second (output tokens / duration in seconds)
	var tps float64
	if duration.Seconds() > 0 && m.Stats.OutputToken > 0 {
		tps = float64(m.Stats.OutputToken) / duration.Seconds()
	}
	log.Infof("relay stats: cache_hit_rate=%.1f%%, cached_tokens=%d, tps=%.1f",
		m.CacheHitRate(), m.Stats.CachedTokens, tps)
...
func (m *RelayMetrics) CacheHitRate() float64 {
	if m.Stats.InputToken == 0 {
		return 0
	}
	return float64(m.Stats.CachedTokens) / float64(m.Stats.InputToken) * 100
}
